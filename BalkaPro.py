...
import Sortament
import BalkaOne as NB
import numpy.linalg as NLS
...
def sbor_dannih():
    global Lkl, Lbk, Lkp, Nv, Nvx1, Nvx2, Moment, MomX, dannie, box_index, OporX, OX
    # sechenie = ui.radioButton.isChecked()
    OporX = vvod('_17', 20, 0)       # координаты промежуточных опор
    ...
    OporX = list(set(OporX))
    OporX.sort()
    OX = [i for i in OporX if i != 0]
...
def sbr_dan():
    # import BalkaOne as NB
    global Lkl, Lbk, Lkp, Nv, Nvx1, Nvx2, Moment, MomX, dannie, box_index, OporX

    Mx_123 = []
    Mx_123.append(NB.raschetOne(Lkl, Lbk, Lkp, Nv, Nvx1, Nvx2, Moment, MomX, privyazki))
    for i in OX:
        Nv = [-1]
        Nvx1 = [i]
        Nvx2 = [0]
...
def raschet():
    ...
    global Lkl, Lbk, Lkp, Nv, Nvx1, Nvx2, MomX, dannie, Ry, Rs, MarkaStali
    global document
    global OporX, OKol
    global Moment, privyazki
    ...
    for i in OX:
        if i < 0:
            error_show('Ошибка ввода координат промежуточных опор')
            return
        if i > sum(Lkl + Lbk + Lkp):
            error_show('Ошибка ввода координат промежуточных опор')
            return
    ...
    try:
        # privyazki = Nvx1 + Nvx2 + xM + OporX + [xR1] + [xR2] + [L]
        privyazki = Nvx1 + Nvx2 + xM + OX + [xR1] + [xR2] + [L]
    except:
    ...
    def simson(X, Y, Lu):
        ...
        b = Y[0]; d = Y[1]; f = Y[2]
        Lu4 = (Lu[-1] - Lu[0])
        sim = Lu4/6 * (a * b + 4 * c * d + e * f)
        return sim
    if sum(OX) == 0: OKol = 0
    '''Если количество прежуточных опор не равно нулю'''
    if sum(OX) != 0:
        OKol = len(OX)
        '''ipuri - список из списков созданных по участкам из грузовой и единичных эпюр от лишних опор'''
        ipuri = sbr_dan()

        '''Создаем спикок из списков от умножения грузовой эпюры на единичные'''
        Mpn = []
        for i in range(1, len(ipuri)):
            xxx = []
            for e in range(0, len(ipuri[i])):
                '''значения умножения на каждом участке'''
                xxx.append(round(simson(ipuri[0][e], ipuri[i][e], Ychastki[e]), 8))
            '''суммируем значения со всех участков'''
            xxx = round(sum(xxx), 8)
            '''добавляем полученные значения в список от умножения грузовой эпюры на каждую единичную'''
            Mpn.append(xxx)
        '''Выдергиваем из общего списка эпюр только единичные эпрюры для перемножения друг на друга и на себя'''
        MabList = ipuri[1:]
        '''Создаем список списков от умножения каждой единичной эпюры на все единичные эпюры, включая себя'''
        Mab = []
        '''Каждую единичную эпюру ...'''
        for i in range(0, len(MabList)):
            xxx = []
            '... умножаем на каждую единичную эпрюру...'
            for u in range(0, len(MabList)):
                yyy = []
                '''... по участкам'''
                for e in range(0, len(MabList[u])):
                    yyy.append(round(simson(MabList[i][e], MabList[u][e], Ychastki[e]), 8))
                '''Суммируем значения со всех перемножения по Симпсону с каждого участка'''
                yyy = sum(yyy)
                '''Добавляем значения в список и переходим на умножение следующей единичной нагрузки на все единичные, включая себя'''
                xxx.append(yyy)
            '''Получаем итоговый списко списков значаний от умножения по Симпсону'''
            Mab.append(xxx)
        Xnn = NLS.solve(Mab, Mpn)
    ...
    if sum(NvP) == 0:
        error_show('Укажите нагрузки на балку')
    R2 = []
    if xR1 == 0:
    ...
    def doubleintegral():
        ...
        for i in range(0, len(XL)):
            ...
            for x in range(0, len(XL[i])):
                ...
                try:
                    # xxxx.append(round( - vP + vQ - vQl + vj + C * XL[i][x] + D, 10))
                    xxxx.append( - vP + vQ - vQl + vj + C * XL[i][x] + D)
                except:
                    # xxxx.append(round( - vP + vQ - vQl + vj, 10))
                    xxxx.append( - vP + vQ - vQl + vj)
            yyy.append(xxxx)
        return yyy
...
# ==========================================================================================================
    ...
    '''Убираем одинаковые числа, это касается нуля при отсутствия консолей'''
    listpriv = [0]
    for i in range(1, len(OxO)):
        if OxO[i] != OxO[i-1]:
            listpriv.append(OxO[i])
        else:
            continue
    OxO = listpriv

    '''Формируем список списков участков между опор'''
    YchOxO = []
    for i in range(1, len(OxO)):
        YchOxO.append([OxO[i-1], OxO[i]])

    '''Создаем списки максимальных и минимальных значений прогибов и моментов Mx по участкам между промежуточных опор'''
    def pereborOxO(IshodList, XL):
        newlist = []
        newXL = []
        for i in YchOxO:
            if i[0] == 0:
                kysok = IshodList[XL.index(i[0]) : XL.index(i[-1]) + 1]
                kys_XL = XL[XL.index(i[0]) : XL.index(i[-1]) + 1]
            if i[0] != 0:
                kysok = IshodList[XL.index(i[0]) + 1 : XL.index(i[-1]) + 1]
                kys_XL = XL[XL.index(i[0]) + 1 : XL.index(i[-1]) + 1]
            x = max(kysok); y = min(kysok)
            if abs(x) > abs(y):
                newlist.append(x)
                newXL.append(kys_XL[kysok.index(x)])
            if abs(x) < abs(y):
                newlist.append(y)
                newXL.append(kys_XL[kysok.index(y)])
        return newlist, newXL

    '''Список макс и мин значений прогибов и их координат'''
    xxx = pereborOxO(EJvII, XL)
    EJvII_OxO     = (xxx)[0]
    EJvII_XL_OxO  = (xxx)[1]
    '''Список макс и мин значений моментов и их координат'''
    xxx = pereborOxO(M_x, XL)
    M_x_OxO       = (xxx)[0]
    M_x_XL_OxO    = (xxx)[1]
    '''Расчетны длины участков между опор'''
    Lychlist = [round(i[1] - i[0], 4) for i in YchOxO]
    '''Определяем наибольшую расчетную длину балки между опорами'''
    lef = round(max(Lychlist) * 1000)

    '''Убираем участки с консолями'''
    # if sum(Lkl) != 0:
    #     M_x_OxO.pop(0)
    #     M_x_XL_OxO.pop(0)
    #     Lychlist.pop(0)
    ...
    class Example(QWidget):
        ...
        def drawBrushes(self, qp):
            ...
            '''Опорные шарниры от промежуточных опор'''
            def ProShar():
                for i in OX:
                    qp.setPen(QPen(QtGui.QColor(0, 100, 150), 1.5, QtCore.Qt.SolidLine))
                    '''Опорные шарниры'''
                    qp.drawEllipse (i * masshtabX + dx - d/2, osBalki + 5, d, d)
                    '''Вертикальные линии'''
                    qp.setPen(QPen(QtGui.QColor(0, 100, 150), 1.5, QtCore.Qt.SolidLine))
                    qp.drawLine(i * masshtabX + dx, osBalki + d + 5, i * masshtabX + dx, osBalki + 25)
                    '''Горизонтальные толстые линии'''
                    qp.setPen(QPen(QtGui.QColor(0, 100, 150), 3, QtCore.Qt.SolidLine))
                    qp.drawLine(i * masshtabX + dx - 10, osBalki + 25, i * masshtabX + dx + 10, osBalki + 25)
                    '''Горизонтальные тонкие линии R1 и R2'''
                    qp.setPen(QPen(QtGui.QColor(0, 100, 150), 1, QtCore.Qt.SolidLine))
                    qp.drawLine(i * masshtabX + dx - 10, osBalki + 30, i * masshtabX + dx + 10, osBalki + 30)
                    '''Стрелки наконечник'''
                    qp.setPen(QPen(QtGui.QColor(255, 85, 0), 2, QtCore.Qt.SolidLine))
                    '''Стрелки наконечник'''
                    '''Стрелки наконечник - уточнить направление после получения значений'''
                    qp.drawLine(i * masshtabX + dx, osBalki + 35, i * masshtabX + dx - 3, osBalki + 43)
                    qp.drawLine(i * masshtabX + dx, osBalki + 35, i * masshtabX + dx + 3, osBalki + 43)
                    qp.drawLine(i * masshtabX + dx - 3, osBalki + 43, i * masshtabX + dx + 3, osBalki + 43)
                    '''Стрелки линия'''
                    qp.drawLine(i * masshtabX + dx, osBalki + 35, i * masshtabX + dx, osBalki + 60)
                    '''Значения промежуточных опор'''
                    qp.setFont(QtGui.QFont("Cambria", 11))
                    qp.drawText(i * masshtabX + dx + 5, osBalki + 45, 50, 30, Qt.AlignLeft, str(abs(round(Xnn[OX.index(i)], 3))))

            if OKol != 0: ProShar()

            '''Опорные шарниры R1 и R2'''
            qp.setPen(QPen(QtGui.QColor(0, 100, 150), 1.5, QtCore.Qt.SolidLine))
            ...
            for i in range(0, len(granX)):
                ...
                # qp.setPen(QPen(QtGui.QColor(0, 100, 150, 150), 2, QtCore.Qt.SolidLine))
                qp.setPen(QPen(QtGui.QColor(0, 0, 0, 150), 1, QtCore.Qt.DashLine))
            ...
            def dataM(a, b, Align, yyy=0):
                ...
                qp.drawText(granXI[a][b] * masshtabX + dx - xxx, osMx + granMI[a][b] * masshtabYM + yyy, 70, 30, Align, str(round(granMI[a][b], 3) if granMI[a][b] != 0 else ''))
            ...
            def dataQ(a, b, Align, yyy=0):
                ...
                qp.drawText(granXI[a][b] * masshtabX + dx - xxx, osQy - granQI[a][b] * masshtabYQ + yyy, 70, 30, Align, str(round(granQI[a][b], 3) if granQI[a][b] != 0 else ''))
            ...
            for a in range(0, len(MImax)):
                for b in range(0, len(MImax[a])):
                    ...
                    qp.drawText(xImax[a][b] * masshtabX + dx - 35, osMx + MImax[a][b] * masshtabYM + yyy, 70, 30, Qt.AlignHCenter, str(round(MImax[a][b], 3) if MImax[a][b] != 0 else ''))
                    qp.setPen(QPen(QtGui.QColor(0, 0, 0), 1, QtCore.Qt.DashLine))
                    qp.drawLine(xImax[a][b] * masshtabX + dx, osMx, xImax[a][b] * masshtabX + dx, osMx + MImax[a][b] * masshtabYM)

            ''' Вертикальные линии и максимальные значения в прогибах '''
            EJvII_OxO_vid = ['%.3e' % i for i in EJvII_OxO] 
            ...
            def dataV(a, Align, yyy=0):
                ...
                if Align == Qt.AlignRight: xxx = 70 
                if Align == Qt.AlignHCenter: xxx = 35
                if EJvII_OxO[a] >= 0: yyy = yyy + 3
                if EJvII_OxO[a]  < 0: yyy = - yyy - 17
                if EJvII_OxO[a] != 0:
                    qp.drawLine(EJvII_XL_OxO[a] * masshtabX + dx, osEJvII, EJvII_XL_OxO[a] * masshtabX + dx, osEJvII + EJvII_OxO[a] * masshtabYEJvII)
                    qp.drawText(EJvII_XL_OxO[a] * masshtabX + dx - xxx, osEJvII + EJvII_OxO[a] * masshtabYEJvII + yyy, 70, 30, Align, str(round(EJvII_OxO[a], 3)) if abs(EJvII_OxO[a]) >= 0.1 else EJvII_OxO_vid[a])
                else:
                    pass
                
            '''Вертикальные линии максимумов по участкам прогибов'''
            if sum(Lkl) == 0.0 and sum(Lkp) == 0.0:
                for i in range(0, len(EJvII_XL_OxO)):
                    dataV( i, Qt.AlignHCenter)

            if sum(Lkl) != 0.0 and sum(Lkp) == 0.0:
                dataV( 0, Qt.AlignLeft)
                for i in range(1, len(EJvII_XL_OxO)):
                    dataV( i, Qt.AlignHCenter)

            if sum(Lkl) == 0.0 and sum(Lkp) != 0.0:
                dataV( -1, Qt.AlignRight)
                for i in range(0, len(EJvII_XL_OxO)-1):
                    dataV( i, Qt.AlignHCenter)

            if sum(Lkl) != 0.0 and sum(Lkp) != 0.0:
                dataV( 0, Qt.AlignLeft)
                dataV( -1, Qt.AlignRight)
                for i in range(1, len(EJvII_XL_OxO)-1):
                    dataV( i, Qt.AlignHCenter)
    ...
    MmaxABS = round(max(abs(Mmax), abs(Mmin)), 3)
    QmaxABS = round(max(abs(Qmax), abs(Qmin)), 3)
    MmaxABS_tsm = MmaxABS
    QmaxABS_ts = QmaxABS
    # Переводим из тс в Н
    ...
    if NDS == 1:    # если упругое-пластичное состояние балки
        ...
        # G = MmaxABS / (Cxm * Wx * Ry * yc)
        if G == 0:
            error_show('Укажите нагрузку на балку')
            return
        
        text_abzac('где:')
        text_abzac('Mx = {} тс м = {} Н мм  -  абсолютное значения изгибающего момента;'.format(MmaxABS_tsm, MmaxABS))
        ...
        if ui.toolBox.currentIndex() != 1:
            ...
            if Mx_xR1 + Mx_xR2 == 0:
                ...
                text_abzac('где:')
                text_abzac('Qy = {} тс = {} H  -  абсолютное значения поперечной силы;'.format(QmaxABS_ts, QmaxABS))
        ...
        if ui.toolBox.currentIndex() == 1:
            ...
            text_abzac('где:')
            text_abzac('Qy = {} тс = {} H  -  абсолютное значения изгибающего момента;'.format(QmaxABS_ts, QmaxABS))
    ...
    if NDS == 0:    # если упругое состояние балки
        ...
        text_abzac('где:')
        text_abzac('Mx = {} тс м = {} H мм  -  абсолютное значения изгибающего момента;'.format(MmaxABS_tsm, MmaxABS))
        text_abzac('Wxn,min = {} мм3 -  момент сопротивления сечения;'.format(Wx))
        text_abzac('Ry = {} Н/мм2  -  расчетное сопротивление растяжению, сжатию, изгибу по пределу текучести;'.format(Ry))
        text_abzac('yc = {}  -  коэффициент условий работы.'.format(yc))
        if G > 1:
    ...
    EJvIIRabsH = round(EJvIIRabs * 9806.65 * 10**9) #(Н мм3)
    RfR = round(EJvIIRabsH / (E * Ix), 3)
    ...
    if sum(Lkl) != 0:
        ...
        EJvIIK1absH = round(EJvIIK1abs * 9806.65 * 10**9) #(Н мм3)
        fRR = round(EJvIIK1absH / (E * Ix), 3)
    ...
    if sum(Lkp) != 0:
        ...
        EJvIIK2absH = round(EJvIIK2abs * 9806.65 * 10**9) #(Н мм3)
        RRf = round(EJvIIK2absH / (E * Ix), 3)
...
def soxranka():
    ...
    if katalog != '':       # при нажатии на кнопку ОТМЕНА, не происходит вылета проги
        ...
        savex = [Lkl, Lbk, Lkp, Nv, Nvx1, Nvx2, Moment, MomX, dannie, OX, box_index]
...
def otkrivalka():
    ...
    if katalog != '':               # при нажатии на кнопку ОТМЕНА, не происходит вылета проги
        ...
        def vvod_open(tablica, stolbec, xList):
            _translate = QtCore.QCoreApplication.translate
            '''Очищаем все ячейки таблицы и узнаем количество её строк'''
